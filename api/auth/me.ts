...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const status = code === 'INVALID_TOKEN' ? 403 : 500;
    return res.status(status).json({ success: false, error: { code, message: code === 'INVALID_TOKEN' ? 'Invalid or expired session token' : (err?.message || 'Unable to load session') } });
  }
}
